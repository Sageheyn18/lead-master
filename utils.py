...
def ensure_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS signals(
            ...
            lat           REAL,
            lon           REAL,
            read_flag     INTEGER DEFAULT 0      -- NEW
        );
...
def cache_summary(url: str, summary: str | None = None) -> str | None:
    ...
    if summary is None:
        row = c.execute("SELECT summary FROM cache WHERE urlhash=?", (_h(url),)).fetchone()
        return None if row is None else row[0]
    c.execute("INSERT OR REPLACE INTO cache VALUES(?,?)", (_h(url), summary))
    c.commit()
