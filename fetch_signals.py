# fetch_signals.py — Lead Master final
...
def _set_cached_raw(seed: str, hits: list[dict]):
    ...
    for h in hits:
        sql = (f"INSERT OR REPLACE INTO {RAW_CACHE_TABLE}"
        ...
        while True:
            try:
                ...
                if "locked" in str(e).lower() and retries>0:
                    ...
                    logging.error(f"DB locked writing raw_hits: {e}")
                    break
    db.commit()
...
def safe_chat(**kw):
    try:
        ...
        logging.warning(f"OpenAI error, skipping: {e}")
        return None
...
def _map_headlines_to_company(headlines: list[str]) -> dict[str,str]:
    prompt = textwrap.dedent("""
      You are an assistant. For each news headline,
      extract the primary COMPANY name. Return JSON
      array of {"headline":"...","company":"..."}.
...
def gpt_summary_batch(headlines: list[str]) -> dict|None:
    prompt = textwrap.dedent("""
      Summarize these headlines in 3 bullets, identify sector,
      confidence (0–1), land_flag (0/1), and primary company.
      Return JSON {summary,sector,confidence,land_flag,company}.
...
def gpt_summary_single(headline: str) -> dict:
    ...
    info = gpt_summary_batch([headline]) or {
        "summary":"","sector":"unknown","confidence":0.0,"land_flag":0,"company":""
    }
...
def manual_search(company: str):
    ...
    if not raw: return {"summary":""}, [], None, None
...
def national_scan():
    ...
    for r in hits:
        co = mapping.get(r["headline"],r["headline"])
        groups[co].append(r)

    db=get_conn()
    for co,recs in groups.items():
        hlines=[r["headline"] for r in recs]
        summ= gpt_summary(co,hlines)
...
def export_pdf(row: dict, summary_text: str, contacts: list[dict]) -> bytes:
    ...
    try:
        ...
        if logo:
            ...
            pdf.image(fn,10,10,20); pdf.set_xy(35,10)
    except: pass

    pdf.set_font("Helvetica","B",16); pdf.multi_cell(0,10,row.get("name","")); pdf.ln(5)
    ...
    pdf.set_y(-20); pdf.set_font("Helvetica","I",9)
