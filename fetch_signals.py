"""
fetch_signals.py  – Lead Master  v5.5   (2025-06-22)

• Restored gpt_signal_info() for national-scan filtering
• dedup() handles both 'title' and 'headline'
• Hybrid permit feeds
• Manual search toggle uses google_news + keyword_filter
...
def dedup(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        txt = r.get("title") or r.get("headline") or ""
        url = r.get("url","")
        t,u = txt.lower(), url.lower()
...
def google_news(co: str, max_rec: int = MAX_HEADLINES) -> list[dict]:
    ...
    if row and now-row[1]<86400:
        ...
        items= [{"title":e.title,"url":e.link,"seendate":today}
...
def fetch_permits(max_rec: int = 10) -> list[dict]:
    ...
    for a in nat: results.append({**a,"src":"national"})
...
def gpt_summary(co: str, heads: list[str]) -> dict:
    ...
    if not rsp: return {"summary":heads[0][:120]+"…","sector":"unknown","confidence":0,"land_flag":0}
    try:
...
def national_scan():
    ...
    for i,kw in enumerate(SEED_KWS[:10],1):
        news=google_news("",MAX_PROSPECTS)
        for a in news:
...
def manual_search(company: str):
    ...
    lat,lon=safe_geocode(heads[0] if heads else "",company)
