# fetch_signals.py — Lead Master v8.0 (2025-06-23)
import os, json, time, datetime, logging, textwrap, requests, sqlite3, csv, threading
...
# Load your NewsAPI key from Streamlit secrets
...
def dedup(rows):
    seen, out = set(), []
    for r in rows:
        key = (r["headline"].lower(), r["url"].lower())
...
def company_contacts(co):
    ...
    if not rsp: return []
    try: return json.loads(rsp.choices[0].message.content)
    except: return []
...
def manual_search(company):
    arts = None
    try:
        arts = api.get_everything(q=company, 
                                  from_param=(datetime.date.today() - datetime.timedelta(days=30)).isoformat(), 
                                  to=datetime.date.today().isoformat(),
                                  language="en", page_size=MAX_HEADLINES
                                 )["articles"]
    except:
        arts = []
    if not arts:
        arts = feedparser.parse(
            "https://news.google.com/rss/search?"
            f"q={quote_plus(company)}%20when:30d"
            "&hl=en-US&gl=US&ceid=US:en"
        ).entries
    arts = [
        {"title":a.title, "url":a.link, "seendate":datetime.datetime.utcnow().strftime("%Y%m%d")}
        for a in arts
        if any(ek in a.title.lower() for ek in EXTRA_KWS)
    ]
    arts = dedup(arts)
    heads = [a["title"] for a in arts][:MAX_HEADLINES]
    if not heads: return None, [], None, None
